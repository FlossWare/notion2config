#!/usr/bin/env bash
# notion-ansible.sh
# Pulls the Compute database from Notion and generates an Ansible inventory
# in INI format with hosts grouped by Type and Location.
#
# Requirements:
#   - curl
#   - jq (>= 1.6)
...
#
# Usage:
#   NOTION_TOKEN=secret_xxx ./notion-ansible.sh
#   NOTION_TOKEN=secret_xxx ./notion-ansible.sh --output inventory.ini
#   NOTION_TOKEN=secret_xxx ./notion-ansible.sh --dry-run
#   NOTION_TOKEN=secret_xxx ./notion-ansible.sh --filter-type web --filter-location datacenter-1
...
while true; do
  ...
  # Append results to accumulated file
...
jq --arg filter "$filter_expr" '[
  .[] |
  {
    ...
    type:     (.properties.Type.select.name // "ungrouped"),
    location: (.properties.Location.select.name // ""),
    os:       (.properties.OS.select.name // ""),
...
generate_inventory() {
  ...
  cat <<EOF
# Ansible inventory — auto-generated by notion-ansible.sh
# Source : Notion Compute database (${DATABASE_ID})
# Generated: ${ts}
# DO NOT EDIT BY HAND — regenerate with notion-ansible.sh
...
EOF
  ...
  for type in $types; do
    # Normalize type for group name (lowercase, alphanumeric + underscore)
    local group_name
    group_name=$(echo "$type" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9_]/_/g')

    echo "[${group_name}]"

    # Print hosts in this type group
    jq -r --arg t "$type" '
      .[] |
      select(.type == $t) |
      .name + " ansible_host=" + .ip +
      (if .mac != "" then " ansible_mac=" + .mac else "" end) +
      (if .os != "" then " ansible_os_family=" + .os else "" end) +
  ...
  cat <<'EOF'
[all:vars]
# Set default SSH user (customize as needed)
# ansible_user=admin
