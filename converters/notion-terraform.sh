#!/usr/bin/env bash
# notion-terraform.sh
# Pulls the Compute database from Notion and generates a Terraform variables file
# in HCL format (tfvars) with compute host definitions.
#
# Requirements:
#   - curl
#   - jq (>= 1.6)
...
#
# Usage:
#   NOTION_TOKEN=secret_xxx ./notion-terraform.sh
#   NOTION_TOKEN=secret_xxx ./notion-terraform.sh --output terraform.tfvars
#   NOTION_TOKEN=secret_xxx ./notion-terraform.sh --dry-run
...
jq --arg filter "$filter_expr" '[
  ...
  '"$filter_expr"'
] | sort_by(.name)' "$TEMP_DIR/all_results.json" > "$TEMP_DIR/hosts.json"
